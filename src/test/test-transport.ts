...
export async function wireTestTransport(
  ...
  const { slot: resolvedSlot } = await wireTransportInternal(store, options);
  return resolvedSlot;
}
...
export async function wireTestTransportWithHub(
  ...
  return wireTransportInternal(store, options);
}
