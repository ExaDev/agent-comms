...
test("createRoom persists a self-signed room:member grant for the owner", async () => {
  ...
  expect(token, "expected a persisted token for the new room").toBeDefined();
  if (token === undefined)
    throw new Error("expected a persisted token for the new room");
  ...
  expect(verdict.claims.capability).toBe("room:member");
  expect(verdict.claims.scope).toEqual({ kind: "room", path: room.id });
  expect(verdict.rootIssuer).toEqual(deviceIdFromHex(owner.id));
});
